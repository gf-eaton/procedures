# how to make VirtualBox Linux VM working in Eaton

Since you are in a unsecure and not able to access to the internet you must copy and paste the script.

### Script

```
#!/bin/bash
#
echo "Enter Your git emailaddress (ie username@eaton.com)"
read ea
echo "Enter Your git username or initials (ie GF)"
read un

# Step 1 certificate
cd
pwd
sudo /bin/mkdir /usr/share/ca-certificates/Eaton
cat > EatonRootCA2.crt <<EOF
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOF

sudo mv EatonRootCA2.crt /usr/share/ca-certificates/Eaton/EatonRootCA2.crt

sudo grep "EatonRootCA2.crt" /etc/ca-certificates.conf
if [ $? -eq 1 ] ; then
  echo "insert new certificate in conf"
  sudo echo "Eaton/EatonRootCA2.crt" >> /etc/ca-certificates.conf
  sudo update-ca-certificates
fi

# Step 2 git
cd
sudo cat > .gitconfig <<EOF
[http]
        proxy = http://proxy.etn.com:8080
        sslVerify = false
[https]
        proxy = http://proxy.etn.com:8080
EOF

git config user.user $un
git config user.email $ea

# Step 3 proxy
echo 'export http_proxy="http://proxy.etn.com:8080"' > /etc/environment
echo 'export https_proxy="http://proxy.etn.com:8080"' >> /etc/environment
echo 'export no_proxy=" localhost, 127.0.0.1"' >> /etc/environment

echo 'Defaults       env_keep+="http_proxy https_proxy no_proxy"' >> /etc/sudoers

# Step testing
cd
sudo apt update
git clone https://github.com/guyfrancoeur/ciclad.git
rm -fr ciclad
wget -O https://raw.githubusercontent.com/gf-eaton/procedures/main/pxmcea-v1.sh
rm pxmcea-v1.sh

echo "if no error on the last 3 commands you are set to go"
```

---
Copyright MIT by GF
